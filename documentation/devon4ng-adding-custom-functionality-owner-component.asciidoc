...
Now we have a logout-button. However we are not redirected to the Login-page. We will therefore create a service that will do this for us. In `app/core` add the files `auth-guard.service.spec.ts` and `auth-guard.service.ts`.
...
To the `owner-overview-page.component.scss` add 
