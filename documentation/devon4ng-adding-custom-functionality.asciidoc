...
image::images/devon4ng/custom-login/root_header.png[Root Header, 250]
...
Finally, we remove the code from `app.component.html` and replace it with a `<router-outlet></router-outlet>` tag. The final result of our root component will look like this:
...
image::images/devon4ng/custom-login/login-page.png[Login Page, 250]
